...
export function EmptyState() {
  return (
    <div
      ...
      className="mx-auto my-8 max-w-[460px] rounded-xl border border-(--border) bg-(--surface) px-7.5 py-8.5 text-center"
    >
      <div className="mx-auto mb-4 flex h-11.5 w-11.5 items-center justify-center rounded-full border border-(--border) text-(--ink-soft)">
      ...
      <p className="m-0 mb-5.5 text-[13px] leading-normal text-(--ink-soft)">
        Looking for{" "}
        <code className="font-mono rounded bg-(--surface-muted) px-1 py-0.5">.cairn/tokens.db</code> — it's empty
        or doesn't exist yet in this project.
      </p>
      ...
      <div className="border-t border-(--border) pt-3.5 text-[10.5px] text-(--ink-soft)">
...
function EmptyStep({ n, text }: { n: number; text: string }) {
  return (
    <div className="flex items-start gap-2.5 text-[12.5px]">
      <span className="font-mono flex h-4.5 w-4.5 flex-none items-center justify-center rounded-full border border-(--border) text-[10px] text-(--ink-soft)">
        {n}
      </span>
      {text}
    </div>
